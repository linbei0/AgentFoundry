...
This is not the primary user experience. HaAgent is a local personal AI assistant: ordinary users should enter a directory, run `haagent`, and configure models inside the TUI with `/model`. `task.yaml` and `haagent run` are kept for reproducible smoke, batch, and eval-oriented workflows.
