...
HaAgent 需要在显式 TUI 入口中提供“快速配置模型”和“快速切换模型”的能力，使用户不用离开 TUI 就能完成 provider/model 发现、API key 配置、profile 保存、当前会话切换、默认 profile 设置和连接测试。

本设计不把 TUI 改成普通默认入口。普通路径仍然是 `haagent setup` 后在任意目录运行 `haagent`；TUI 仍然是显式入口 `haagent tui`。TUI 模型中心复用用户级 profile 和 runtime 模型边界，不创建一套独立配置系统。
