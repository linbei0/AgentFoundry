...
上一版路线已经确认自然语言会话应成为主入口，但理解仍偏开发者 CLI。新的目标需要更明确：HaAgent 是本地个人 AI 助手，用户配置一次模型后，进入任意目录运行 `haagent`，围绕当前目录完成文件阅读、资料整理、文档修改、项目分析、命令执行和多轮任务延续。当前代码中，`haagent` 仍默认进入文本 chat，`haagent tui` 作为显式 TUI 垂直切片入口已经可用。
...
```powershell
uv run haagent chat "帮我理解这个项目" --workspace-root E:\some-project --profile deepseek
uv run haagent chat --resume session-1234abcd
...
`haagent` 是普通用户的第一个前端；`haagent chat` 是同一会话核心的显式/高级入口。
...
- `:quit` / `:exit`：退出。
- `:new`：清空当前会话上下文。
- `:status`：显示当前 session、session package、workspace、provider。
- 后续再考虑 `:abort`，因为真正取消正在执行的 shell/model call 需要 runtime 支持。
...
payload 只放前端展示和状态判断需要的摘要，不放完整工具输出、完整文件内容、完整用户答案、完整 episode trace 或 secret。CLI 和 TUI 都只消费 `ChatEvent` 并渲染，不实现 Agent loop、工具执行或 episode 读取逻辑。

当前事件契约已稳定，显式 `haagent tui` 垂直切片已实现；本地 Web、ACP bridge 和其他前端暂不实现。
...
- `haagent chat --resume <session_id-or-path>` 可恢复 bounded session summary、turn_count 和 workspace_root。
...
- 新增 `haagent setup`，写入用户级 profile 和 active profile。
- 无子命令 `haagent` 默认进入个人助手聊天模式。
- `haagent chat` 保留为显式自然语言入口。
- 支持 REPL 输入多条任务。
...
- 输出简洁工具进度和最终结果。
- 每条用户任务写 episode。
- 新增 `haagent sessions` 和 `haagent --continue` 服务目录会话恢复。

验收：

- 能在任意文件夹运行 `uv run haagent`。
- 首次使用前通过 `uv run haagent setup` 配置默认模型连接。
- 不需要 `task.yaml`。
- 能处理不改代码的任务。
- 能处理文档修改任务。
- 能处理代码修改 + 命令验证任务。
...
- 引入 `AgentSession`。
- REPL 内保存 session history summary。
- 支持 `:status`、`:new`、`:quit`。
- 输出 turn 级进度。
...
- `haagent` 或 `haagent chat` 无位置参数时进入 REPL，支持 `:status`、`:new`、`:quit` / `:exit`。
...
- `haagent --continue` / `haagent chat --continue` 可恢复当前目录最近会话。
- `haagent chat --resume <session_id-or-path>` 可恢复 turn_count、workspace_root、bounded session summary 和 working_state。
...
状态：事件契约、轻量 session package 和显式 `haagent tui` 垂直切片已先行稳定；本地 Web、ACP bridge 暂不实现。

任务：

- 基于已稳定的前端无关事件流继续收敛最小前端适配层。
- 打磨显式 TUI 垂直切片；本地 Web/Desktop bridge 后续再评估。
...
> 实现 `haagent setup`、无子命令 `haagent`、active profile、当前目录 workspace、sessions/continue，并保留 `haagent chat` 作为显式入口。覆盖 CLI 测试，并运行 `uv run pytest -q` 和 `uv run haagent check`。
...
> 在 `haagent` / `haagent chat` 上保持 REPL 模式和最小 session shell，支持 `:quit`、`:status`、`:new`，每条用户输入复用单次 chat 执行路径，并维护 bounded session summary。

第三条实现任务（已完成）：

> 抽出 `AgentSession` 和事件流，让 CLI 不直接承载会话逻辑，为后续 TUI/桌面/ACP bridge 做准备。

说明：`AgentSession` 已抽出；`ChatEvent` 契约已稳定为前端无关事件流；显式 `haagent tui` 垂直切片已接入同一服务层。后续可在不改变 Agent loop 的前提下继续打磨 TUI，或接本地 Web / ACP bridge。

## 10. 待确认问题

1. 是否需要首次启动提示工作区边界和高风险工具审批语义？
2. REPL 是否需要中断正在执行的任务，还是只支持轮次间退出？
3. 显式 TUI 垂直切片是否继续打磨，还是先评估本地 Web / ACP bridge？
